...
function SectionHeading({ T, children, right }) {
  return (
...
export default function UploadECG() {
  ...
  const TABS = [
    ...
    {key:"grid",        label: "Grid Crop"},
    {key:"detections",  label: "All Detections"},
    {key:"overlay",     label: t('tab_overlay')},
    {key:"leadcrops",   label: "Lead Segments"},
  ...
  const onAnalyze = async () => {
    ...
    try {
      ...
      if(json.status==="success"){ setResult(json.data); setSelLead(json.data.lead_list?.[0]||null); setTab("medical"); }
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const durationText = result.computed_duration ? `${result.computed_duration}s`
      ...
      if (canvas) {
        ...
        const margin = 14; const maxPdfWidth = pageWidth - (margin * 2);
        const pdfHeight = maxPdfWidth / (canvas.width / canvas.height);
  ...
  const tabBase = { flex: "1 1 auto", textAlign: "center", padding:"7px 14px", borderRadius:9, border:"none", cursor:"pointer", fontSize:11, fontWeight:600, fontFamily:"'DM Sans',sans-serif", transition:"all .15s" };

  const imgPanel = (src, label) => src ? (
    <div>
      <div style={{fontSize:11,fontWeight:600,color:T.textMuted,marginBottom:8,letterSpacing:.5}}>{label}</div>
      <div style={{background:T.cardBg,border:`1.5px solid ${T.cardBorder}`,borderRadius:12,overflow:"hidden",
                   boxShadow:"0 2px 12px rgba(0,0,0,.07)",cursor:"zoom-in"}}
           onClick={()=>setLightboxImg(src)}>
        <img src={src} alt={label} style={{width:"100%",display:"block",objectFit:"contain",maxHeight:480}}/>
      </div>
    </div>
  ) : null;
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:T.pageBg, minHeight:"100vh", padding:"clamp(16px, 4vw, 32px)", width: "100%", boxSizing: "border-box", overflowX: "hidden" }}>
      <style>{`
        ...
        @keyframes ecgFadePulse { 0%,100%{opacity:.7;transform:scale(.98)}50%{opacity:1;transform:scale(1.02)} }
        @keyframes modalFadeIn  { from{opacity:0;transform:translateY(10px) scale(.98)}to{opacity:1;transform:translateY(0) scale(1)} }
      ...
      <div style={card}>
        <div style={{...cardTtl,display:"flex",alignItems:"center",gap:6}}>
          {t('upload_select')}
          <button onClick={()=>setShowQualityModal(true)}
            ...
            <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke={T.textMuted} strokeWidth="2.5"
              onMouseOver={e=>e.currentTarget.style.stroke=T.accent} onMouseOut={e=>e.currentTarget.style.stroke=T.textMuted}>
        ...
        <div style={{display:"flex",gap:12,alignItems:"stretch",flexWrap:"wrap"}}>
          ...
          <button onClick={onAnalyze} disabled={loading||!file}
            style={{flex:"1 1 auto",justifyContent:"center",padding:"13px 30px",borderRadius:10,border:"none",
                    background:loading||!file?"#e5e7eb":`linear-gradient(135deg,${T.accent},${T.accentHover})`,
                    color:loading||!file?"#9ca3af":"#fff",fontWeight:700,fontSize:13,
                    cursor:loading||!file?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:8,
                    boxShadow:loading||!file?"none":`0 2px 14px ${T.accent}44`,transition:"all .18s",whiteSpace:"nowrap"}}>
            {loading
          ...
          <button onClick={()=>window.open('https://wa.me/918884774504?text='+encodeURIComponent('Hi! I would like to digitize an ECG image.'),'_blank')}
            style={{flex:"1 1 auto",justifyContent:"center",padding:"13px 30px",borderRadius:10,border:"none",background:"#25D366",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:8,boxShadow:`0 2px 14px rgba(37,211,102,.3)`,transition:"all .18s",whiteSpace:"nowrap"}}>
        ...
        {error && (
          <div style={{marginTop:16,padding:"14px 18px",borderRadius:12,background:"#fef2f2",border:"1px solid #fecaca",display:"flex",alignItems:"center",gap:12,boxShadow:"0 4px 14px rgba(220,38,38,.08)"}}>
            ...
            <div style={{color:"#991b1b",fontSize:13.5,fontWeight:600}}>
              {(()=>{
                let d=error;
                if(typeof error==='string'){const m=error.match(/"detail"\s*:\s*"([^"]+)"/);if(m&&m[1])d=m[1];}
                return typeof d==='string' ? d.charAt(0).toUpperCase()+d.slice(1) : d;
              })()}
            </div>
          </div>
        )}
      ...
      {loading && (
        <div style={{textAlign:"center",padding:"60px 20px",display:"flex",flexDirection:"column",alignItems:"center",gap:24,background:T.cardBg,borderRadius:16,border:`1px solid ${T.cardBorder}`,boxShadow:T.cardShadow}}>
          ...
          <div style={{fontSize:16,fontWeight:700,color:T.accent||"#2563eb",animation:"ecgFadePulse 1.5s ease-in-out infinite",letterSpacing:.5}}>
        ...
        <>
          ...
          <div style={{display:"flex",gap:14,marginBottom:20,flexWrap:"wrap",width:"100%"}}>
            {[
              ...
              {label:"Valid Leads",       val: result.valid_leads_count ?? "—"},
            ].map(s=>(
          ...
          {tab==="medical" && (
            <div>
              <SectionHeading T={T} right={
                <button onClick={handleDownloadPDF} disabled={isPdfGenerating}
                  style={{padding:"7px 14px",background:isPdfGenerating?"#9ca3af":T.accent,color:"#fff",border:"none",borderRadius:8,fontSize:11,fontWeight:700,cursor:isPdfGenerating?"wait":"pointer",display:"flex",alignItems:"center",gap:6,boxShadow:isPdfGenerating?"none":`0 2px 10px ${T.accent}33`,transition:"all .2s"}}>
                  <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                  </svg>
                  {isPdfGenerating?"Generating...":"Download Report"}
                </button>
              }>Output Image — Full 12-Lead ECG</SectionHeading>
          ...
          {tab==="grid" && (
            <div>
              <SectionHeading T={T}>Grid Region Detected</SectionHeading>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:16}}>
                {imgPanel(previewUrl, "Original Photo")}
                {imgPanel(result.grid_crop_image, "Extracted Grid Region")}
              </div>
              {result.grid_box && (
                <div style={{marginTop:12,padding:"10px 14px",background:T.inputBg,borderRadius:8,border:`1px solid ${T.divider}`,fontSize:12,color:T.textMuted,fontFamily:"'DM Mono',monospace"}}>
                  Grid bbox: [{result.grid_box.join(", ")}]
                </div>
              )}
            </div>
          )}
          ...
          {tab==="detections" && (
            <div>
              <SectionHeading T={T}>YOLO Row + Lead Detections</SectionHeading>
              <p style={{fontSize:13,color:T.textMuted,marginBottom:14}}>
                Blue = row boxes · Green = lead boxes · Purple = mapped lead labels
              </p>
              {result.detections_image
                ? imgPanel(result.detections_image, "All Detections on Grid Crop")
                : <div style={{color:T.textMuted,fontSize:13}}>No detection visualization available.</div>
              }
            </div>
          )}

          {/* ── Lead Segment Crops (masks) ── */}
          {tab==="leadcrops" && (
            <div>
              <SectionHeading T={T}>13 Lead Segments — nnUNet Masks</SectionHeading>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12}}>
                {result.lead_list?.map(lead=>{
                  const img = result.lead_crop_images?.[lead];
                  const color = LEAD_COLORS[lead]||T.accent;
                  return (
                    <div key={lead} style={{background:T.cardBg,border:`1.5px solid ${T.cardBorder}`,borderRadius:12,overflow:"hidden",boxShadow:T.cardShadow}}>
                      <div style={{padding:"8px 12px",background:`${color}14`,borderBottom:`1px solid ${color}30`,display:"flex",alignItems:"center",gap:8}}>
                        <span style={{fontSize:14,fontWeight:700,fontFamily:"'DM Mono',monospace",color}}>{lead}</span>
                        <span style={{fontSize:10,color:T.textMuted,marginLeft:"auto"}}>
                          {_is_flat_js(result.signals?.[lead]) ? "⚠ flat" : "✓ signal"}
                        </span>
                      </div>
                      {img
                        ? <img src={img} alt={`Lead ${lead} mask`}
                            style={{width:"100%",display:"block",background:"#000",cursor:"zoom-in"}}
                            onClick={()=>setLightboxImg(img)}/>
                        : <div style={{height:80,display:"flex",alignItems:"center",justifyContent:"center",color:T.textMuted,fontSize:12}}>No mask</div>
                      }
                    </div>
                  );
                })}
          ...
          {tab==="overlay" && (
            <div>
              <SectionHeading T={T}>13-Lead Signal Overlay</SectionHeading>
          ...
          {tab==="leads" && (
            <div>
              <SectionHeading T={T}>All Leads — Individual Waveforms</SectionHeading>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(255px,1fr))",gap:12}}>
                {result.lead_list?.map(lead=>{
                  ...
                  return (
                    <div key={lead} onClick={()=>setSelLead(sel?null:lead)}
                      style={{background:sel?`${T.accent}0e`:T.cardBg,border:`1.5px solid ${sel?T.accent:T.cardBorder}`,borderRadius:13,padding:"14px 15px",cursor:"pointer",transition:"all .16s",boxShadow:sel?`0 4px 16px ${T.accent}22`:T.cardShadow}}>
          ...
          {tab==="original" && previewUrl && (
            <div>
              <SectionHeading T={T}>Original ECG Image</SectionHeading>
      ...
      {lightboxImg && (
        <div onClick={()=>setLightboxImg(null)}
          style={{position:"fixed",top:0,left:0,right:0,bottom:0,backgroundColor:"rgba(15,23,42,.85)",backdropFilter:"blur(6px)",zIndex:10000,overflowY:"auto",padding:"40px 20px",animation:"backdropFadeIn .2s ease-out"}}>
          <button onClick={()=>setLightboxImg(null)}
            style={{position:"fixed",top:20,right:20,background:"rgba(15,23,42,.6)",backdropFilter:"blur(4px)",border:"1px solid rgba(255,255,255,.2)",borderRadius:"50%",width:44,height:44,display:"flex",justifyContent:"center",alignItems:"center",cursor:"pointer",color:"#fff",zIndex:10001}}>
          ...
          <div onClick={e=>e.stopPropagation()}
            style={{position:"relative",width:"100%",maxWidth:1200,margin:"0 auto",background:"#fff",padding:12,borderRadius:12,boxShadow:"0 25px 50px -12px rgba(0,0,0,.3)",animation:"modalFadeIn .3s cubic-bezier(.16,1,.3,1)"}}>
            <img src={lightboxImg} alt="ECG detail" style={{width:"100%",height:"auto",display:"block",borderRadius:8}}/>
          </div>
        </div>
      )}
      ...
      {/* Quality Modal */}
      {showQualityModal && (
        <div onClick={()=>setShowQualityModal(false)}
          style={{position:"fixed",top:0,left:0,right:0,bottom:0,backgroundColor:"rgba(15,23,42,.65)",backdropFilter:"blur(4px)",zIndex:9999,display:"flex",justifyContent:"center",alignItems:"flex-start",padding:"clamp(20px,5vw,40px) 20px",overflowY:"auto",animation:"backdropFadeIn .2s ease-out"}}>
          <div onClick={e=>e.stopPropagation()}
            style={{background:T.cardBg,borderRadius:20,padding:"clamp(20px,4vw,32px)",maxWidth:720,width:"100%",boxShadow:"0 25px 50px -12px rgba(0,0,0,.25)",position:"relative",margin:"auto",animation:"modalFadeIn .3s cubic-bezier(.16,1,.3,1)"}}>
            <button onClick={()=>setShowQualityModal(false)}
              style={{position:"absolute",top:16,right:16,background:T.inputBg,border:`1px solid ${T.divider}`,borderRadius:"50%",width:32,height:32,display:"flex",justifyContent:"center",alignItems:"center",cursor:"pointer",color:T.textMuted}}>
            ...
            <h2 style={{margin:"0 0 8px",color:T.textPrimary,fontSize:20,fontWeight:700}}>Image Quality Guidelines</h2>
            <p style={{color:T.textMuted,fontSize:14.5,lineHeight:1.6,marginBottom:28}}>
              Please upload images of at least this quality so the AI can detect and digitize the signals properly.
              The system now automatically detects and crops the ECG grid region before processing — so background
              clutter, table surfaces, and other objects are handled. Make sure the ECG grid is clearly visible and not blurry.
            </p>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))",gap:20}}>
              {[{src:"/norm.png",label:"Example 1: Colored ECG"},{src:"/normal+ECG.webp",label:"Example 2: B&W ECG"}].map(({src,label})=>(
                <div key={src} style={{background:T.inputBg,borderRadius:12,padding:12,border:`1px solid ${T.divider}`}}>
                  <div onClick={()=>setSelectedExampleImage(src)}
                    style={{width:"100%",height:180,borderRadius:8,overflow:"hidden",background:"#fff",display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid #f1f5f9",cursor:"zoom-in"}}>
                    <img src={src} alt={label} style={{width:"100%",height:"100%",objectFit:"contain",padding:8}}/>
                  </div>
                  <div style={{textAlign:"center",fontSize:13,fontWeight:600,color:T.textSecondary,marginTop:12}}>{label}</div>
                </div>
              ))}
            </div>
            <div style={{marginTop:28,display:"flex",justifyContent:"flex-end"}}>
              <button onClick={()=>setShowQualityModal(false)}
                style={{background:T.accent,color:"#fff",border:"none",padding:"10px 24px",borderRadius:10,fontWeight:600,fontSize:14,cursor:"pointer",boxShadow:`0 4px 12px ${T.accent}40`}}>
                Got it
              </button>
            </div>
          </div>
      ...
      {selectedExampleImage && (
        <div onClick={()=>setSelectedExampleImage(null)}
          style={{position:"fixed",top:0,left:0,right:0,bottom:0,backgroundColor:"rgba(15,23,42,.8)",backdropFilter:"blur(6px)",zIndex:10000,overflowY:"auto",padding:"40px 20px",animation:"backdropFadeIn .2s ease-out"}}>
          <button onClick={()=>setSelectedExampleImage(null)}
            style={{position:"fixed",top:20,right:20,background:"rgba(15,23,42,.6)",backdropFilter:"blur(4px)",border:"1px solid rgba(255,255,255,.2)",borderRadius:"50%",width:44,height:44,display:"flex",justifyContent:"center",alignItems:"center",cursor:"pointer",color:"#fff",zIndex:10001}}>
            <svg width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
          <div onClick={e=>e.stopPropagation()}
...
/* helper used inside JSX without importing the backend fn */
function _is_flat_js(sig) {
  if(!sig||!sig.length) return true;
  const v = sig.filter(x=>x!==null&&!isNaN(x));
  if(!v.length) return true;
  return Math.max(...v)-Math.min(...v) < 0.05;
}